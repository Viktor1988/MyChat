...
//  MyChat
//
//  Created by Виктор Попов on 06.08.2021.
...
class ListenerService {
    ...
    func waitingChatsObserve(chats: [MChat], completion: @escaping (Result<[MChat],Error>) -> Void) -> ListenerRegistration? {
        ...
        let chatsListener = chatsRef.addSnapshotListener { (qeuerySnapshot, error) in
            ...
            snapshot.documentChanges.forEach { (diff) in
                ...
                switch diff.type {
                 case .added:
                     ...
                     chats .append(chat)
//                    self.notify.scheduleNotificationAwairingChats(chat: chat)
                 case .modified:
        ...
        let messageListener = ref.addSnapshotListener { (querySnapshot, error) in
            ...
            snapshot.documentChanges.forEach { (diff) in
                ...
                case .added:
                    ...
                    self.notify.scheduleNotification(chat: chat, message: message)
                case .modified:
                    break
                case .removed:
                    break
